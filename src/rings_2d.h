/*
 * Simplified 2D version of "Rings", faster to calculate than the full 3D version.
 *
...
class Rings2D : public Effect
{
public:
    Rings2D(const char *palette, float seed)
        : palette(palette),
          seed(seed),
          d(0, 0, 0),
          threshold(-1.0)
    ...
    static const float xySpeed = 0.6;
    static const float xyScale = 0.08;
    ...
    virtual void beginFrame(const FrameInfo &f)
    {
        ...
        float speed = pow(fabsf(noise2(f.time * 0.01, seed + 40.5)), 2.5) * xySpeed;
    ...
    {
        float dist = sq(p.point[0] - center[0]) + sq(center[1]) + sq(p.point[2] - center[2]);
        Vec3 pulse = Vec3(sinf(d[2] + dist * spacing) * ringDepth, 0, 0);
        Vec3 s = Vec3(p.point[0] * xyScale, p.point[2] * xyScale, seed) + d;

        float n = (fbm_noise3(s + pulse, 4) + threshold) * brightnessContrast;
        if (n > 0.0f) {
            // Positive brightness is possible

            const Vec3 chromaOffset = Vec3(0, 0, 10);
            float m = fbm_noise3(s + chromaOffset, 2) * colorContrast;
